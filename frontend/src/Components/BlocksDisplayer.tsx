...
function ImageBlockDisplay({ data }: ImageBlock) {
  const splitUrl = data.file?.url?.split("/");
  ...
  return (
    <div>
      {fileName && (
        <>
          <div>
            <CloudinaryImage imageName={fileName} />
          </div>
          {data.caption && <p className="text-sm">{data.caption}</p>}
        </>
      )}
    </div>
  );
...
function getBlockDisplay(block: EditorBlock, key: string) {
  switch (block.type) {
    ...
    case BlockType.image:
      return <ImageBlockDisplay key={key} {...block} />;
    default:
      return null;
  }
}
